...
"use strict"; // const random1 = Math.floor(Math.random() * 45) + 1; => 1부터 45까지의 랜덤 숫자
// 랜덤한 로또번호 생성 로직
...
var n = 6;
var lottoarr = new Array(n).fill(0); // 배열길이 6으로 생성

for (var i = 0; i < n; i++) {
  lottoarr[i] = Math.floor(Math.random() * 45) + 1; // i번째에 랜덤한 번호 넣기(범위는 1부터 45까지)

  lottoarr = _toConsumableArray(new Set(lottoarr)); // 중복된 로또번호 제거
...
lottoarr = lottoarr.sort(function (a, b) {
  return a - b;
});
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "4664" + '/');
